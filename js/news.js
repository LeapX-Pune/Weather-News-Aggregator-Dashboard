...
export function getNewsFixture({ category = 'all', countryCode } = {}) {
  const filter = CATEGORY_MAP[category] || CATEGORY_MAP.all;
  let articles = BASE_ARTICLES.filter(filter).map((a) => ({
    ...a,
    local: countryCode ? a.country === countryCode : false,
  ...
  return { articles, category, countryCode, total: articles.length };
}
